...
\item The status of any primal or dual solution returned by \HiGHS is
  ...
  \HighsInfoPrimalSolutionStatus and \HighsInfoDualSolutionStatus. The
  validity of any basis information is indicated by the value of the
  \code{HighsInfo} entry \HighsInfoBasisValidity. These are
  interpreted using the values in
  Table~\ref{tab:HighsSolutionStatusBasisValidity}.
  \begin{table}[ht]
    \leftline{
      \begin{tabular}{|l|l|p{8cm}|}\hline
        ...
        2 & \code{kSolutionStatusFeasible} &A feasible solution is available\\\hline
        0 & \code{kBasisValidityInvalid} &A valid basis is not available\\
        1 & \code{kBasisValidityValid} &A valid basis is available\\\hline
    \end{tabular}}
    \caption{Interpreting solution status values}\label{tab:HighsSolutionStatusBasisValidity}
  \end{table}
  
\item \HiGHS can now be used to presolve an LP or MIP, allowing users
  to extract the model. \HiGHS can also determine the optimal primal
  solution, dual solution or basis for the original model, given the
  corresponding data for the optimal solution of the presolved model.
\end{itemize}
...
style guide~\cite{GoogleStyleGuide}, numerous constants have changed
name. Those used in the \Cpp interface are listed in
...
Within \Cpp, the original \HighsLp{} class was stretched to define
MIPs, but a QP can't be argued to be a generalisation of LP!  Hence we
have introduced a \HighsModel{} class that can be used to define an
LP, MIP or QP. A \HighsLp{} instance is a member of he class
\HighsModel{} class, allowing all uses of \HighsLp{} to be
maintained. Within the \code{Highs} class, a \HighsModel{} may be
...
was originally the only format permitted by \HiGHS. However, it
is often preferable for modelling interfaces and users to specify the
constraint matrix row-wise. Hence the \HighsLp{} class has an
\Aformat{} member to specify whether the matrix is held
...
\HighsModel{} instance. There is a \C method \code{Highs\_getModel}
to obtain the current model.
...
statuses that can occur have changed for some edge cases. In
particular, and consistent with commercial solvers, if a model is both
primal and dual infeasible, \HiGHS no longer identifies this: it will
only indicate primal infeasibility. Again, as with commercial solvers,
if a model is dual infeasible then, unless it is identified as being
either primal infeasible, or primal unbounded, the \HiGHS model status
will indicate it as being ``infeasible or unbounded''.

Within \Cpp, the model status value is taken from an \code{enum class}
\HighsModelStatus. Since the names of constants in \HiGHS now conform
to the Google \Cpp style guide~\cite{GoogleStyleGuide}, the entries of
\HighsModelStatus have changed. For the \C interface, entries of
\HighsModelStatus are cast to integers and, in general, the value
associated with a given status has changed. The correspondence between
current and former model status values is set out in
...
Any optimal objective is at least a given bound &11&\code{kObjectiveBound}&
10&\code{REACHED\_DUAL\_OBJECTIVE\_VALUE\_UPPER\_BOUND}\\
...
\HighsLogFile. The production of log output and name of the logging
file is controlled by the \HighsOptions{} settings in
Table~\ref{tab:HighsLogging}.
\begin{table}[ht]
\leftline{
  \begin{tabular}{|l|l|l|p{10cm}|}\hline
    Option name & Type & Default & Description\\\hline
    \OutputFlag & \Bool & \true & Determines whether there is logging output from \HiGHS\\
    \LogToConsole & \Bool & \true & Determines whether any logging output from \HiGHS goes to the console\\
    \LogFile & \String & \HighsLogFile & Allows the name of the logging file to be changed
...
have been deleted from \HighsOptions. When \HiGHS is run as an
executable, these options are not redundant, so the corresponding
command-line options are retained.

The options \SolutionFile, \WriteSolutionToFile
and \WriteSolutionPretty have been retained in
\HighsOptions, but are only relevant when \HiGHS is run as an
...
are listed in Table~\ref{tab:DeprecatedMethodsCpp}. The first two
relate to the old logging system, and both now set the option
\OutputFlag{} to false, stopping any logging from \HiGHS: they will
soon be deleted. Early versions of \HiGHS had a dedicated method to
get the simplex iteration count, and this was also returned by two
...
to \HiGHS by beginning ``\code{Highs\_}''. Deprecated methods in the
\C interface are listed in Table~\ref{tab:DeprecatedMethodsC}
...
\code{getNumRows} &\code{Highs\_getNumRows}&\code{getNumRow} &\code{Highs\_getNumRow}\\
\code{getNumCols} &\code{Highs\_getNumCols}&\code{getNumCol} &\code{Highs\_getNumCol}\\
\code{getNumEntries} &&\code{getNumNz} &\\\hline
