...
\HiGHS so that it uses long (64-bit) integers in all cases. This is
achieved by setting the \code{CMake} parameter
\code{-DHIGHSINT64=on}. The type \code{HighsInt} is then set to be
\code{int64\_t} rather than \code{int}. Long integers are not
currently supported by the \code{FORTRAN} interface. Since \HiGHS can
be compiled with either standard or long integers, integer values are
...
interface, \code{Highs\_passLp} now has a parameter to indicate
whether the matrix is passed row-wise or column-wise.
...
names, together with integer values and short description are given in
...
within the resources available, a small number of methods have been
deleted, and a rather larger number are deprecated. These can be
expected to be deleted in future versions of \HiGHS.
