...
built within \HiGHS by adding columns and rows to an empty model. The
definition of the \HiGHS model data structure is set out in
...
\MatrixFormat&\Aformat&Format of the constraint matrix\\
...
Three members of the \HighsLp{} class (\Aformat, \Sense{} and
...
\begin{table}[ht]
  \leftline{
    \begin{tabular}{|l|r|p{6cm}|}\hline
      ...
      \MatrixFormatNone&0&Undefined matrix format\\
      \MatrixFormatColwise&1&Column-wise matrix format\\
      \MatrixFormatRowwise&2&Row-wise matrix format\\\hline
...
constraint matrix column-wise, this was originally the only format
permitted by \HiGHS. However, it is often preferable for modelling
interfaces and users to specify the constraint matrix row-wise. The
\Aformat{} member of the \HighsLp{} class is used to specify whether
the matrix is held row-wise or column-wise. When a \HighsLp{} instance
is created, the format is indicated as undefined by being set to
\MatrixFormatNone. If the format is not set to either
\MatrixFormatColwise{} or \MatrixFormatRowwise, \HiGHS returns an
error. In the \C interface, an \HighsInt{} parameter indicates whether
the matrix is row-wise or column-wise according to the integer values
in Table~\ref{tab:HighsLpEnumClass}.
...
\subsection{Reading from a data file}
...
\HiGHS has a single method for solving the incumbent model. After this
is run, the solution data that is available from \HiGHS comes from
four sources: the model status, a set of scalar information values,
the solution of the model and the status of each variable and
constraint.
...
\HighsInt & \HighsInfoBasisValidity &Status of the variable and constraint status information\\
...
available, consult the value of \HighsInfoBasisValidity{} via \HighsInfo{}
...
\subsubsection{Solving the model}\label{sect:SolvingTheModel}
...
\begin{tabbing}
  \CHighsRunDef{}
\end{tabbing}
By default, \HiGHS will use what it considers to be the best solver and run-time
option settings for the incumbent model. To force \HiGHS to use a
particular solver and/or run-time options, \HighsOptions{} values defined in
Section~\ref{sect:Options} may be modified.
...
\leftline{
  \setlength\tabcolsep{0pt}
  \begin{tabular}{lp{13cm}}
    \HighsInt{} \code{Highs\_getDoubleInfoValue(}&\VoidStarHighsInstance\codeComma\Const{} \CharStar{} \HighsInfoName\codeComma \DoubleStar{} \HighsInfoValue\code{);}\\
  \end{tabular}}
\codelinespace
...
\leftline{
  \setlength\tabcolsep{0pt}
  \begin{tabular}{lp{13cm}}
    \HighsInt{} \code{Highs\_getSolution(}&\VoidStarHighsInstance\codeComma\\
    &\DoubleStar{} \HighsSolutionColValue\codeComma\DoubleStar{} \HighsSolutionColDual\codeComma\\
    &\DoubleStar{} \HighsSolutionRowValue\codeComma\DoubleStar{} \HighsSolutionRowDual\code{);}\\
  \end{tabular}}
\codelinespace
...
\leftline{
  \setlength\tabcolsep{0pt}
  \begin{tabular}{lp{13cm}}
    \HighsInt{} \code{Highs\_getBasis(}&\VoidStarHighsInstance\codeComma
    \HighsIntStar{} \HighsBasisColStatus\codeComma
    \HighsIntStar{} \HighsBasisRowStatus\code{);}\\
  \end{tabular}}
